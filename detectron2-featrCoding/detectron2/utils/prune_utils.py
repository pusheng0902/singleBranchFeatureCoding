...
def L1_norm_row(layer):
    weight_copy = layer.weight.data.abs().clone().cpu().numpy()
    norm = np.sum(weight_copy, axis=(0,2,3))
    return norm
...
def CSS_row(layer, k):
    '''
    k: pruning rate, i.e. select (1-k)*C rows
    '''
    weight = layer.weight.data.detach()
    X = torch.transpose(weight, 0, 1) # (in_ch, out_ch, kernel_H, kernel_W)
    X = weight.view(weight.shape[0], -1) # (in_ch, -1)
    X = torch.transpose(X, 0, 1) # (-1, in_ch)
    # X = weight.view(weight.shape[0], -1)
...
def init_channel_mask(model, ratio):
    ...
    for m in model.modules():
        ...
        layer_id += 1
        if layer_id != 7 and layer_id != 10: continue
        
        in_channels = m.weight.data.shape[1]
        num_keep = int(in_channels * (1 - ratio))
        rank = np.argsort(L1_norm_row(m))
...
def apply_channel_mask(model, cfg_mask):
    ...
    for m in list(model.modules())[::-1]:
        ...
        if conv_count in l1_inv:
            mask = cfg_mask[layer_id_in_cfg].float().cuda()
            mask = mask.view(1,m.weight.data.shape[1],1,1)
            m.weight.data.mul_(mask)
            layer_id_in_cfg -= 1

        if conv_count in l2_inv:
            # print(torch.sum(cfg_mask[layer_id_in_cfg+1].float().cuda()))
            prev_mask = F.interpolate( cfg_mask[layer_id_in_cfg+1].unsqueeze(0).unsqueeze(1), scale_factor=4, mode="nearest" ).float().cuda()
            # print(torch.sum(prev_mask))
            prev_mask = prev_mask.view(m.weight.data.shape[0],1,1,1)
...
def IS_update_channel_mask(model, layer_ratio_up, layer_ratio_down, old_model):
    ...
    idx = -1
    cfg_mask = []
    for [m, m0] in zip(list(model.modules())[::-1], list(old_model.modules())[::-1]):
        if isinstance(m, nn.Conv2d):
            layer_id += 1
            in_channels = m.weight.data.shape[1]
            
            if layer_id in l1_inv:
                idx += 1
                # number of channels
                num_keep = int(in_channels*(1-layer_ratio_down[idx]))
                num_free = int(in_channels*(1-layer_ratio_up[idx])) - num_keep
                # pruning criterion
                rank = np.argsort(CSS_row(m, layer_ratio_down[idx])) # (512,)
                selected = rank[::-1][:num_keep] # in_ch * 0.5
                freedom = rank[::-1][num_keep:] # in_ch - in_ch * 0.5
                # restore MRU
                copy_idx = np.where(L1_norm_row(m) == 0)[0]
                w = m0.weight.data[:, copy_idx.tolist(), :, :].clone()
                m.weight.data[:,copy_idx.tolist(),:,:] = w.clone()
                # importance sampling
                weight_copy = m.weight.data.detach().cpu()
                weight_copy = torch.transpose(weight_copy, 0, 1) # in_ch, out_ch, k_H, k_W
                weight_copy = weight_copy.contiguous().view(weight_copy.shape[0], -1) # (in_ch, -1)
                # contiguous is necessary, or else a runtime error will be raised
                # or simply use reshape
                weight_copy = torch.transpose(weight_copy, 0, 1) # (-1, in_ch)
            ...
            if layer_id in l2_inv:
                l1 = copy_idx*4
                l2,l3,l4 = l1+1,l1+2,l1+3
                prev_copy_idx = (l1).tolist() + (l2).tolist() + (l3).tolist() + (l4).tolist()
                w = m0.weight.data[prev_copy_idx,:,:,:].clone()
                m.weight.data[prev_copy_idx,:,:,:] = w.clone()
            
    prev_model = deepcopy(model)
    return cfg_mask[::-1], prev_model
